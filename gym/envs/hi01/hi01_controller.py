...
from gym.envs.hi01.hi01_controller_config import Hi01ControllerCfg
from gym.utils.math import *
from gym.envs import LeggedRobot
from isaacgym import gymapi, gymutil
import numpy as np
from typing import Tuple, Dict
from .hi01_utils import (
    FootStepGeometry,
    SimpleLineGeometry,
    VelCommandGeometry,
    smart_sort,
...
class Hi01Controller(LeggedRobot):
    ...
    def _reset_system(self, env_ids):
        ...
        self.step_commands[env_ids, 0] = self.env_origins[env_ids] + torch.tensor(
            [0.0, -0.075, 0.0], device=self.device
    ...
    def _calculate_foot_states(self, foot_states):
        ...
        foot_height_vec = (
            torch.tensor([0.0, 0.0, -0.03055]).repeat(self.num_envs, 1).to(self.device)
        )
    ...
    def check_termination(self):
        ...
        # print("self.base_lin_vel: ",torch.any(
        #     torch.norm(self.base_lin_vel, dim=-1, keepdim=True) > 10.0, dim=1
        # ))
        self.terminated |= torch.any(
            torch.norm(self.base_ang_vel, dim=-1, keepdim=True) > 10.0, dim=1
        )
        # print("self.base_ang_vel: ",torch.any(
        #     torch.norm(self.base_ang_vel, dim=-1, keepdim=True) > 10.0, dim=1
        # ))
        ...
        # print("self.projected_gravity: ",torch.any(
        ...
        # print("self.base_pos: ",torch.any(self.base_pos[:, 2:3] < 0.3, dim=1))
